...
public class NioSender extends AbstractSender {
    ...
    protected boolean read(SelectionKey key) throws IOException {
        ...
        if (ackbuf.doesPackageExist() ) {
            ...
            boolean ack = Arrays.equals(ackcmd,com.firefly.wcreate.turnip.tribes.transport.Constants.ACK_DATA);
            boolean fack = Arrays.equals(ackcmd,com.firefly.wcreate.turnip.tribes.transport.Constants.FAIL_ACK_DATA);
            if ( fack && getThrowOnFailedAck() ) throw new RemoteProcessException("Received a failed ack:com.firefly.wcreate.turnip.tribes.transport.Constants.FAIL_ACK_DATA");
            return ack || fack;
        } else {
            return false;
        }
     ...
     *
     * @throws IOException
     * TODO Implement this com.firefly.wcreate.turnip.tribes.transport.IDataSender method
     */
    @Override
